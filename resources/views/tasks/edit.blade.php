...
@section('inhoud')
    <div class="bg-white p-8 rounded-lg shadow-lg max-w-lg mx-auto">
        ...
        <form action="{{ route('tasks.update', $task->id) }}" method="POST" class="flex flex-col">
            @csrf
            @method('PUT')
            ...
            <div class="mb-4">
                ...
                <input type="text" id="taak" name="taak" 
                       ...
                       value="{{ old('taak', $task->taak) }}" required>
                @error('taak')
            ...
            <div class="mb-4">
                ...
                <input type="text" id="status" name="status" 
                       class="border border-gray-300 rounded p-2 w-full" 
                       value="{{ old('status', $task->status) }}" required>
                @error('status')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                <textarea id="omschrijving" name="omschrijving" 
                          class="border border-gray-300 rounded p-2 w-full">{{ old('omschrijving', $task->omschrijving) }}</textarea>
            </div>
            ...
            <button type="submit" class="w-full py-2 bg-blue-600 text-white rounded-md text-lg hover:bg-blue-700 transition duration-200">Wijzigingen Opslaan</button>
        </form>
    </div>
@endsection
